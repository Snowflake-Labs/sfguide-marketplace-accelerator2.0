...
create procedure if not exists sp_mpadmin_Plisting_sendtoconsumer (LISTING_NAME varchar, CONSUMER_NAME varchar, CONSUMER_ACCOUNTLOCATOR varchar, CONSUMER_ACCOUNTREGION varchar, LICENSE_START_DATE DATE, LICENSE_END_DATE DATE, OVERRIDE_SHARE_RESTRICTION_FLAG BOOLEAN)
    returns string
    language javascript
    execute as caller
    as
    ...
    // Set variables
    var mp_listing_name = LISTING_NAME.toUpperCase();
    var mp_overridesharerestriction_flag = OVERRIDE_SHARE_RESTRICTION_FLAG;
    var mp_listing_consumer_name = CONSUMER_NAME.toUpperCase();
  ...
  result=snowflake.execute({ sqlText:  `select basedatabase_name from `+mp_admin_tbl_basedboverride_qualified+` where listing_name = '`+LISTING_NAME+`';`});
    ...
    var mp_consumer_dbname = mp_database_name;
    var mp_consumer_accountlocator= CONSUMER_ACCOUNTLOCATOR.toUpperCase();
    ...
    var entitlement_tablesequence_qualified = entitlement_dbname.concat('.').concat(entitlement_schemaname).concat('.SEQ_ENTITLEMENT');
    var entitlement_licenseStartDate = LICENSE_START_DATE.toLocaleDateString("en-US");  
    var entitlement_licenseEndDate = LICENSE_END_DATE.toLocaleDateString("en-US") ;  
    

//set consumer account region. Replace public. string if exists
var result = snowflake.execute({ sqlText:  `select REPLACE('`+CONSUMER_ACCOUNTREGION.toUpperCase()+`','PUBLIC.');` });  
result.next();
var mp_consumer_accountregion = result.getColumnValue(1);

//check if provided consumer account region is valid
snowflake.execute({ sqlText:  `SHOW REGIONS;`  });  
var result = snowflake.execute({ sqlText:  `select "snowflake_region" from table(result_scan(last_query_id())) where "snowflake_region" ='`+mp_consumer_accountregion+`';`  });  
var valid_region_check=result.next();

//set provider region to curren region. Replace public. string if exists
var result = snowflake.execute({ sqlText:  `select REPLACE(current_region(),'PUBLIC.');` });  
result.next();
var provider_currentregion = result.getColumnValue(1);

//check if entitlemenet table and share exists. if not error
try {
var describe_share_result =snowflake.execute({ sqlText:  `DESCRIBE SHARE `+mp_consumer_share_name });  
var countobjects_in_share = describe_share_result.getRowCount();

var result = snowflake.execute({ sqlText:  `select 1 from `+entitlement_tablename_qualified+` where ENTITLEMENT_CONTAINER_NAME = '`+mp_consumer_share_name+`' and  ENTITLEMENT_CONTAINER_NAME = 'SHARE' and CONSUMER_SNOWFLAKE_ACCOUNTLOCATOR = '`+mp_consumer_accountlocator+`';`  });  
var existing_entitlement_check=result.next();

} catch (err)  {return "Call procedures mpadmin.util.sp_mpadmin_listing_setup() and mpadmin.util.sp_mpadmin_Plisting_addobject() before continuing. Failed with error: " + err;   }    



//check if entitlement exists for given share and consumer account then error
if (existing_entitlement_check)
{
    return 'Consumer Account ('.concat(mp_consumer_accountlocator).concat(') is already added to the Share (').concat(mp_consumer_share_name).concat(')')
}
  //check if provided region is a valid region else Error
  else if(!valid_region_check)
  {
      return 'Provided consumer account region is invalid. Run command: SHOW REGIONS and choose appropriate value from snowflake_region column '.concat(valid_region_check)
  } 
  //check if provided region matches currentregion of procedure else Error
    else if (provider_currentregion!=mp_consumer_accountregion)
    {
        return 'Consumer account region is not in the region of this account. Run this procedure on account in region: '.concat(mp_consumer_accountregion)
    }
      //check if share contains data objects other than database and schema else Error
      else if (countobjects_in_share <=2)
      {
          return 'There are 0 objects in share ('.concat(mp_consumer_share_name ).concat("). Call procedure mpadmin.util.sp_mpadmin_Plisting_addobject() before continuing.")
      }

         else
          {

              // capture contents of share for proof of delivery
              var rs_row_num = 1;
              var rs_rows_as_json = {};
                while (describe_share_result.next())  {
                  // skip first 2 objects which are database and schema in describe output
                  if (rs_row_num > 2) {
                    rs_rows_as_json[rs_row_num-2] = { };
                    rs_rows_as_json[rs_row_num-2].object_type = describe_share_result.getColumnValue(1);
                    rs_rows_as_json[rs_row_num-2].object_name = describe_share_result.getColumnValue(2);
                    }
                  ++rs_row_num;
                  }
              // generate record key for entitlement table
              var result = snowflake.execute({ sqlText:  `select `+ entitlement_tablesequence_qualified +`.nextval from dual` });  
              result.next();
              var entitlement_key = result.getColumnValue(1);

              // insert entilement record
              var insert_command1 = `insert into `+entitlement_tablename_qualified+` (ENTITLEMENT_KEY,CONSUMER_NAME, CONSUMER_SNOWFLAKE_ACCOUNTLOCATOR, CONSUMER_SNOWFLAKE_ACCOUNTREGION, ENTITLEMENT_CONTAINER_NAME ,ENTITLEMENT_CONTAINER_TYPE ,ENTITLEMENT_EFF_DATE, ENTITLEMENT_EXP_DATE)
                                      values (`+entitlement_key+`,'`+mp_listing_consumer_name+`','`+mp_consumer_accountlocator+`','`+mp_consumer_accountregion+`','`+mp_consumer_share_name+`','SHARE','`+entitlement_licenseStartDate+`','`+entitlement_licenseEndDate+`')`;

            // add account to share. 
              if(mp_overridesharerestriction_flag)
              {var share_command1 = `alter share `+mp_consumer_share_name+` add accounts = `+mp_consumer_accountlocator+` share_restrictions=false`;}
              else {var share_command1 = `alter share `+mp_consumer_share_name+` add accounts = `+mp_consumer_accountlocator;}

              try {snowflake.execute ({sqlText: share_command1}); //add consumer to share
                     snowflake.execute ( {sqlText: insert_command1}); //if adding consumer to share is successful; insert into entitlement table
                    }
                catch (err)  {return "Failed: " + err;   
              }      


       success_msg =  "Listing Share sucessfully sent to Consumer"
       success_msg += "\n ================================================================="
       success_msg += "\n Share sent: ".concat(mp_consumer_share_name)
       success_msg += "\n Sent to Consumer Account: ".concat(mp_consumer_accountlocator).concat(" in region: ").concat(mp_consumer_accountregion)
       success_msg += "\n\n Objects in the share: ".concat(JSON.stringify(rs_rows_as_json))
       success_msg += "\n To double-check objects in share, run: DESCRIBE SHARE ".concat(mp_consumer_share_name).concat(" ;")
       return success_msg
      } 
...
create procedure if not exists sp_mpadmin_Plisting_sendtoconsumer (LISTING_NAME varchar, CONSUMER_NAME varchar, CONSUMER_ACCOUNTLOCATOR varchar, CONSUMER_ACCOUNTREGION varchar, LICENSE_START_DATE DATE, LICENSE_END_DATE DATE)
    returns string
    language javascript
    execute as caller
    as
    $$
       var entitlement_licenseStartDate = LICENSE_START_DATE.toLocaleDateString("en-US");  
       var entitlement_licenseEndDate = LICENSE_END_DATE.toLocaleDateString("en-US") ;  
       result=snowflake.execute({ sqlText:  `call MPADMIN.util.sp_mpadmin_Plisting_sendtoconsumer  ('`+LISTING_NAME+`','`+CONSUMER_NAME+`','`+CONSUMER_ACCOUNTLOCATOR+`','`+CONSUMER_ACCOUNTREGION+`','`+entitlement_licenseStartDate+`','`+entitlement_licenseEndDate+`',0);`});
       result.next();
        return result.getColumnValue(1);
    $$ ;
...
create procedure if not exists sp_mpadmin_Plisting_sendtoconsumer_revoke (LISTING_NAME varchar, CONSUMER_NAME varchar, CONSUMER_ACCOUNTLOCATOR varchar, CONSUMER_ACCOUNTREGION varchar, LICENSE_START_DATE DATE, LICENSE_END_DATE DATE, OVERRIDE_SHARE_RESTRICTION_FLAG BOOLEAN)
    returns string
    language javascript
    execute as caller
    as
...
try {
var result = snowflake.execute({ sqlText:  `select 1 from `+entitlement_tablename_qualified+` where ENTITLEMENT_CONTAINER_TYPE = 'SHARE' and ENTITLEMENT_CONTAINER_NAME = '`+mp_consumer_share_name+`' and CONSUMER_SNOWFLAKE_ACCOUNTREGION ='`+mp_consumer_accountregion+`' and  CONSUMER_SNOWFLAKE_ACCOUNTLOCATOR = '`+mp_consumer_accountlocator+`';`  });  
var existing_entitlement_check=result.next();
}  catch (err)  {return "Nothing to revoke. Failed with error: " + err;   }   
...
if (!existing_entitlement_check)
{
    return 'Consumer Account ('.concat(mp_consumer_accountlocator).concat(') is not added to the Share (').concat(mp_consumer_share_name).concat(')')
}
  //check if provided region is a valid region else Error
         ...
         else
          {
              ...
              // delete entilement record
              var delete_command1 = `delete from `+entitlement_tablename_qualified+` where ENTITLEMENT_CONTAINER_TYPE = 'SHARE' and ENTITLEMENT_CONTAINER_NAME = '`+mp_consumer_share_name+`' and CONSUMER_SNOWFLAKE_ACCOUNTREGION ='`+mp_consumer_accountregion+`' and  CONSUMER_SNOWFLAKE_ACCOUNTLOCATOR = '`+mp_consumer_accountlocator+`';`
            // remove account from share
              var share_command1 = `alter share `+mp_consumer_share_name+` remove accounts = `+mp_consumer_accountlocator;

              try {
                     snowflake.execute ({sqlText: share_command1});
                     snowflake.execute ( {sqlText: delete_command1});
                  }
              catch (err)  {
                  return "Failed: " + err;   // Return a success/error indicator.
              }      
       ...
       success_msg =  "Listing Share sucessfully removed for Consumer"
       success_msg += "\n ================================================================="
       success_msg += "\n Consumer Account: ".concat(mp_consumer_accountlocator).concat(" in region: ").concat(mp_consumer_accountregion)
       success_msg += "\n removed from Share: ".concat(mp_consumer_share_name)
...
create procedure if not exists sp_mpadmin_Plisting_sendtoconsumer_revoke (LISTING_NAME varchar, CONSUMER_NAME varchar, CONSUMER_ACCOUNTLOCATOR varchar, CONSUMER_ACCOUNTREGION varchar, LICENSE_START_DATE DATE, LICENSE_END_DATE DATE)
    returns string
    language javascript
    execute as caller
    as
    $$
       var entitlement_licenseStartDate = LICENSE_START_DATE.toLocaleDateString("en-US");  
       var entitlement_licenseEndDate = LICENSE_END_DATE.toLocaleDateString("en-US") ;  
       result=snowflake.execute({ sqlText:  `call MPADMIN.util.sp_mpadmin_Plisting_sendtoconsumer_revoke  ('`+LISTING_NAME+`','`+CONSUMER_NAME+`','`+CONSUMER_ACCOUNTLOCATOR+`','`+CONSUMER_ACCOUNTREGION+`','`+entitlement_licenseStartDate+`','`+entitlement_licenseEndDate+`',0);`});
       result.next();
        return result.getColumnValue(1);
    $$ ;     
